"""
prebid: PbjsVideoHeroPlayerPlacementTest
...
_JS_TEMPLATE = (Path(__file__).parent.parent / "js" / "pbjs_video_hero_player_placement.js").read_text()

class PbjsVideoHeroPlayerPlacementTest(VideoOnlyTest):

    name = "PbjsVideoHeroPlayerPlacementTest"
    ...
    async def execute(self, page, url: str) -> TestResult:
        ...
        js = _JS_TEMPLATE.replace("__HERO_ADUNIT_CODE__", self.HERO_ADUNIT_CODE)
    ...
    async def validate(self, result: TestResult) -> TestResult:
        ...
        for bidder in sorted(per_bidder.keys()):
            ...
            if miss_p == 0 and inv_p == 0 and miss_c == 0 and inv_c == 0:
            ...
            if inv_p:
                reasons.append(f"invalid placement={inv_p} (expected {diag.get('expectedPlacement', 1)})")
            if miss_c:
                reasons.append(f"missing plcmt={miss_c}")
            if inv_c:
                reasons.append(f"invalid plcmt={inv_c} (expected {diag.get('expectedPlcmt', 2)})")
        ...
        result.metadata.update(
            {
                ...
                "expected_plcmt": diag.get("expectedPlcmt"),
