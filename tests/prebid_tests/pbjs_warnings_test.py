...
_JS_INIT = (Path(__file__).parent.parent / "js" / "pbjs_warnings_init.js").read_text()
...
class PbjsPrebidWarningsTest(BaseTest):
    ...
    async def execute(self, page, url: str) -> TestResult:
        ...
        init = await page.evaluate(_JS_INIT)
        ...
        if self.config.get("trace"):
            ...
            print(
                "[PbjsPrebidWarningsTest] execute diag:",
                {
                    "url": url,
    ...
    async def validate(self, result: TestResult) -> TestResult:
        ...
        if not init.get("hasPbjs"):
            ...
            result.warnings.append("pbjs not present on page; cannot run PbjsPrebidWarningsTest.")
            return result
