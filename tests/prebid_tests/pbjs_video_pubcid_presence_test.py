...
- SKIPPED: window.pbjs not present, or no VIDEO hero_player bids captured (no-bid round).
"""
from pathlib import Path
from core.base_test import VideoOnlyTest, TestResult, TestState

_JS = (Path(__file__).parent.parent / "js" / "pbjs_video_pubcid_presence.js").read_text()
...
class PbjsVideoPubcidPresenceTest(VideoOnlyTest):
    ...
    async def execute(self, page, url: str) -> TestResult:
        ...
        if int(data.get("heroBidsConsidered") or 0) == 0:
            result.state = TestState.SKIPPED
            result.warnings.append("No VIDEO hero_player bids captured — no-bid round; skipping pubcid check.")
            return result
